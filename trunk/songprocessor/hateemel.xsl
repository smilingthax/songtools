...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
 ...
 <xsl:template name="songcontent">
   ...
   <xsl:variable name="akk_inline" select="set:leading($tree1[self::rep|self::quote]/*,$tree1[self::rep|self::quote]/br[1])[self::akk]"/>
   <xsl:if test="($tree1[self::akk][1] or $akk_inline) and not($noakk)">
     <div class="akk1">
<!-- TODO: use akk_inline[special only akk ver] instead of $tree1 -->
...
<!--     <xsl:when test="count($inNodes[self::br])=0 and (../self::vers or ../self::refr or ../self::bridge or ../self::ending) and not(following-sibling::node())"/>-->
     <xsl:when test="not($inNodes)"/>
 ...
 <xsl:template match="rep|quote" mode="do-akks"><!-- match alle inline-tags! -->
   <xsl:param name="out_style"/>
 ...
 <xsl:template match="rep">
   ...
   <xsl:call-template name="songcontent">
     ...
     <xsl:with-param name="indent"><xsl:copy-of select="$indent"/>
       <xsl:if test="$indent">&#160;</xsl:if><xsl:value-of select="$part1"/></xsl:with-param>
...
<!-- TODO: gescheit -->
   <xsl:call-template name="rep_it">
     <xsl:with-param name="inNodes">&#160;</xsl:with-param>
     <xsl:with-param name="anz">30</xsl:with-param>
   </xsl:call-template>
 </xsl:template>
